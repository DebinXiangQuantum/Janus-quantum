...
def circuit_to_dag(circuit, copy_operations=True, *, qubit_order=None, clbit_order=None):
    ...
    # Use Janus's native circuit_to_dag (doesn't support all params yet, so we ignore them for now)
    dagcircuit = _janus_circuit_to_dag(circuit)
    # TODO: Handle copy_operations, qubit_order, clbit_order if needed

    # Set metadata if circuit has these attributes
    if hasattr(circuit, '_duration'):
        dagcircuit._duration = circuit._duration
    if hasattr(circuit, '_unit'):
