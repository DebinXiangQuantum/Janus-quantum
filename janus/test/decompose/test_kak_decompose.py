...
def parse_gate_from_json(file_path):
    """从JSON文件解析门信息"""
    with open(file_path, 'r') as f:
        data = json.load(f)
    
    # 检查数据格式并提取门信息
    # 格式可能是 [[gate_info]] 或者 {'circuit': [gate_info]}
    gate_info = None
    
    if isinstance(data, list) and len(data) > 0 and isinstance(data[0], list) and len(data[0]) > 0:
        # 格式为 [[gate_info]]
        gate_info = data[0][0]
    elif isinstance(data, dict) and 'circuit' in data and len(data['circuit']) > 0:
        # 格式为 {'circuit': [gate_info]}
        gate_info = data['circuit'][0]
    else:
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Test KAK decomposition')
    parser.add_argument('--file', type=str, required=True, help='JSON file containing the gate')
    parser.add_argument('--output', type=str, help='Output JSON file for circuit representation')
    parser.add_argument('--euler-basis', type=str, default='ZXZ', help='Euler decomposition basis (default: ZXZ)')
    ...
    if not args.output:
        ...
        default_output_filename = f"{file_stem}_kak_decomposed{file_suffix}"
    ...
    # 解析门信息
    gate = parse_gate_from_json(args.file)
    print(f"Decomposing {gate.name.upper()} gate using KAK decomposition (from file: {args.file})")
    
    # 执行KAK分解
    try:
        decomposed_circuit = decompose_kak(gate, euler_basis=args.euler_basis)
        
        print(f"KAK decomposition successful!")
        ...
        print(f"\n✓ {gate.name.upper()} gate KAK decomposition completed successfully!")
