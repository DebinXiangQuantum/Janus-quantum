...
# Stub for deprecate_func decorator
def deprecate_func(**kwargs):
    """Stub decorator for deprecation warnings"""
    def decorator(func):
        return func
    return decorator
...
class KAKDecomposition:
    r"""KAK decomposition for two-qubit unitaries (also known as Weyl decomposition).

    Decompose two-qubit unitary using the KAK (Kraus-Albert-Kosut) decomposition:
    ...
    _specializations = _two_qubit_decompose_accel.Specialization
    ...
    def __init__(
        ...
        *,
        _specialization: _two_qubit_decompose_accel.Specialization | None = None,
    ):
        unitary_matrix = np.asarray(unitary_matrix, dtype=complex)
        self._inner_decomposition = _two_qubit_decompose_accel.TwoQubitWeylDecomposition(
    ...
    def actual_fidelity(self, **kwargs) -> float:
        ...
        return _two_qubit_decompose_accel.trace_to_fid(trace)
    ...
    def from_bytes(
        ...
        **kwargs,
        ...
        """Decode bytes into :class:`.KAKDecomposition`."""
        # Used by __repr__
...
class ControlledUKAKDecomposer:
    r"""Decompose two-qubit unitary in terms of a desired
    :math:`U \sim U_d(\alpha, 0, 0) \sim \text{Ctrl-U}`
    gate that is locally equivalent to an :class:`.RXXGate` using KAK decomposition."""
...
class KAKBasisDecomposer:
    ...
    def __init__(
        ...
        self._inner_decomposer = _two_qubit_decompose_accel.TwoQubitBasisDecomposer(
            gate,
            Operator(gate).data,
    ...
    def decomp0(target):
        ...
        return _two_qubit_decompose_accel.TwoQubitBasisDecomposer.decomp0(target)
...
class _LazyTwoQubitCXDecomposer(KAKBasisDecomposer):
    __slots__ = ("_inner",)
    ...
    def _load(self):
        if self._inner is None:
            self._inner = KAKBasisDecomposer(CXGate())
...
"""
This is an instance of :class:`.KAKBasisDecomposer` that always uses
...
TwoQubitWeylDecomposition = KAKDecomposition
TwoQubitBasisDecomposer = KAKBasisDecomposer
TwoQubitControlledUDecomposer = ControlledUKAKDecomposer
