...
def main():
    ...
    parser = argparse.ArgumentParser(description='Test single gate decomposition')
    parser.add_argument('--file', type=str, required=True, help='JSON file containing the matrix or circuit')
    parser.add_argument('--basis', type=str, default='U', help='Basis for decomposition (default: U)')
    ...
    # 检查是否为单比特门分解
    if matrix.shape != (2, 2):
        raise ValueError(f"Expected 2x2 matrix for single qubit gate, got {matrix.shape}")
    
    # 验证矩阵是否为酉矩阵
    if not is_unitary_matrix(matrix):
        print("Warning: Matrix is not unitary!")
    
    # 使用OneQubitEulerDecomposer进行分解
    try:
        decomposer = OneQubitEulerDecomposer(basis=args.basis)
        decomposed_circuit = decomposer(matrix)
