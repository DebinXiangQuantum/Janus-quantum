"""
Tech2: Gate Fusion Optimization
Usage: python test/optimize/test_tech2_gate_fusion.py --file benchmark/opt_3.json
"""
...
from janus.compiler import MergeRotationsPass
from janus.optimize import SingleQubitGateOptimizer, SingleQubitRunCollector, InverseGateCanceller
...
def create_large_scale_circuit():
    ...
    return qc, "Large scale gate fusion test circuit (200 qubits, ~10000 gates)"
...
def run_optimization_test(qc, description):
    ...
    print(f"\n{'='*70}")
    print(f"Tech2 Gate Fusion Test")
    print(f"{'='*70}")
    print(f"Description: {description}")
    print(f"API: MergeRotationsPass + InverseGateCanceller")
...
def main():
    parser = argparse.ArgumentParser(description='Tech2: Gate Fusion Optimization')
    parser.add_argument('--file', type=str, help='Input circuit JSON file path')
    args = parser.parse_args()

    print("="*70)
    print("Tech2: Gate Fusion Optimization")
    print("="*70)
